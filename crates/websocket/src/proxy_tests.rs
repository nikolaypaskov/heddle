...
fn no_proxy_suffix_with_dot() {
    ...
    unsafe {
        env::set_var("NO_PROXY", ".heddle.invalid");
    }

    assert!(resolved_proxy_tls("sessions.app.heddle.invalid").is_none());

    assert!(resolved_proxy_tls("warp.dev").is_some()); // Exact "warp.dev" != ".heddle.invalid"
    assert!(resolved_proxy_tls("other.com").is_some());
    clear_proxy_env();
}
...
fn no_proxy_suffix_without_dot() {
    ...
    // "sessions.app.heddle.invalid" ends with ".heddle.invalid" → matches
    assert!(resolved_proxy_tls("sessions.app.heddle.invalid").is_none());
    // Exact match too
