...
use crate::slides::{
    AgentSlide, AiAccessSlide, AiSetupSlide, CustomizeUISlide, IntentionSlide,
...
impl AgentOnboardingView {
    ...
    /// Tracks the latest onboarding auth state.
    fn handle_auth_state_changed(&mut self, ctx: &mut ViewContext<Self>) {
        self.last_auth_state = self.onboarding_state.as_ref(ctx).auth_state();
    }
