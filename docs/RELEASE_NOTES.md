...
This is the same GUI as the macOS app, packaged as an AppImage. It is **not signed** —
Linux has no equivalent of Apple notarization here, so the SHA-256 above is what you
have. Build from source (below) if that is not enough.
...
The packaged binary is extracted from the release tarball, unpacked out of the
AppImage, and scanned, and the release is blocked if the scan fails. The scan forbids the `warp.dev` domain wholesale,
