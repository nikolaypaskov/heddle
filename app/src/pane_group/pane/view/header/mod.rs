...
impl<P: BackingView> PaneHeader<P> {
    ...
    fn render_right_justified_row(
        ...
        // The row to the left of the required controls used to hold the sharing button and the
        // view-only indicator, and nothing else. Both are gone, so it is not built at all --
        // an empty `Shrinkable`/`Clipped` wrapper would occupy no space but still allocate on
        // every header render.
    ...
    /// Adds overlay children to the stack. The overflow menu is the only one left; the
    /// sharing dialog that used to share this slot is gone.
