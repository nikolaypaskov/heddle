...
/// A pane has no actions of its own.
///
/// The only one was `ShareContents`, bound to "Share pane", which published the pane's
/// backing object to Warp Drive. `PaneView` is still registered with
/// `add_typed_action_view`, so it must name an action type; this one is uninhabited, which
/// is the type-level statement that no such action can be dispatched.
#[derive(Debug, Clone)]
pub enum PaneAction {}
...
impl<P: BackingView> TypedActionView for PaneView<P> {
    ...
    fn handle_action(&mut self, action: &Self::Action, _ctx: &mut ViewContext<Self>) {
        // `PaneAction` is uninhabited, so this is unreachable by construction.
        match *action {}
    }
}
