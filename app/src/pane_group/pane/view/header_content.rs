...
///
/// This used to carry a `'a` lifetime, solely to hold a borrowed closure that rendered the
/// pane's sharing controls. Sharing published the pane's contents to Warp's server, which
/// this build does not reach, so the closure and the lifetime with it are gone; every field
/// here is owned.
