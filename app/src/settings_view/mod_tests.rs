...
fn arrow_down_from_account_with_collapsed_agents_lands_on_first_subpage() {
    ...
    // not skip over the collapsed umbrella to the next top-level stop.
...
#[test]
fn arrow_up_from_code_umbrella_with_collapsed_agents_lands_on_last_subpage() {
    let nav_items = realistic_nav_items();
    let stops = build_nav_stops(&nav_items, |_| true);

    // Pressing Up from the Code umbrella (entered via CodeIndexing) should land
    // on the collapsed Agents umbrella, which resolves to ThirdPartyCLIAgents
    // (last visible subpage) so the user continues moving in natural reading
    // order rather than being jumped back to the top of the umbrella.
    let next = simulate_cycle(
        &nav_items,
        &stops,
...
fn arrow_up_into_collapsed_umbrella_respects_search_filter_for_last_subpage() {
    ...
    // From the Code umbrella (entered via CodeIndexing), Up should land on the
    // last *visible* AI subpage (AgentMCPServers), not on the filtered-out
...
fn arrow_down_from_expanded_last_subpage_leaves_umbrella() {
    ...
    // ThirdPartyCLIAgents is the last Agents subpage; Down should move to the
    // Code umbrella, which auto-expands to its first subpage CodeIndexing.
    let next = simulate_cycle(
        &nav_items,
        &stops,
        SettingsSection::ThirdPartyCLIAgents,
...
fn arrow_down_across_adjacent_collapsed_umbrellas() {
    ...
    // From the Agents umbrella (entered via ThirdPartyCLIAgents), Down should
    // land on the first Code subpage (Code umbrella auto-expands).
