...
use super::ai_query_routing::conversation_failed_before_task_creation;
...
impl TerminalView {
    ...
    /// Whether an ended cloud/ambient pane should surface the conversation-ended tombstone.
    ///
    /// Heddle (FOSS): the cloud task cache is gone, so a pane that carries an ambient task id is
    /// always non-resumable here; the only other tombstone case is a conversation that errored
    /// out before any run existed.
    pub(in crate::terminal::view) fn should_show_conversation_ended_tombstone(
        &self,
        ctx: &AppContext,
    ) -> bool {
        {
        ...
        conversation_failed_before_task_creation(self.id(), BlocklistAIHistoryModel::as_ref(ctx))
    }
    ...
    pub fn on_session_share_joined(
        ...
        let is_ambient = self.is_ambient_agent_session();
        self.input().update(ctx, |input, ctx| {
            input
    ...
    pub fn on_session_share_ended(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.should_show_conversation_ended_tombstone(ctx) || should_insert_legacy_tombstone {
    ...
    pub(crate) fn insert_conversation_ended_tombstone(&mut self, ctx: &mut ViewContext<Self>) {
