    ...
    fn subtext(&self, appearance: &Appearance) -> Option<Box<dyn Element>> {
        let text: Cow<str> = match self {
            ...
            TerminationType::Premature { shell_detail, .. } => format!(
                "Something went wrong while starting {shell_detail} and Heddlifying it, causing the \
                process to terminate. Heddlify script output is displayed here, which may point at \
                a cause."
            )
            .into(),
        };
                    ...
                    ui_builder
                        ...
                        .on_click(|ctx, _, _| {
                            ctx.dispatch_typed_action(Action::OpenUrl(
                                "https://github.com/nikolaypaskov/heddle/issues/new".to_string(),
                            ));
                        })
                        .finish(),
                    ui_builder
