...
impl TerminalView {
    ...
    /// Re-renders the pane header for the active agent view conversation.
    ///
    /// This used to also compute a `ShareableObject` for the header's share button. Sharing a
    /// conversation or a session published it to Warp's server, so both the button and the
    /// object it described are gone; the header still needs refreshing when the agent view
    /// enters, exits, or changes conversation.
...
impl TerminalView {
    ...
    pub fn is_ambient_agent_session(&self) -> bool {
        false
    }
    ...
    pub fn is_cloud_agent_session(&self) -> bool {
        self.is_ambient_agent_session() || self.model.lock().is_cloud_agent_conversation()
    }
    ...
    pub fn selected_conversation_status(&self, ctx: &AppContext) -> Option<ConversationStatus> {
        ...
        let Some(conversation) = self.selected_conversation_for_user_facing_chrome(ctx) else {
            ...
            if long_running && self.is_ambient_agent_session() {
