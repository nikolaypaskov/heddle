...
fn test_secrets_serialization() {
    let mut blockgrid = mock_blockgrid("foo user@heddle.invalid bar");
...
fn test_find_url_omits_trailing_periods() {
    ...
    let blockgrid = mock_blockgrid("Visit https://github.com/example/repo/issues.");
    assert_eq!(
        blockgrid
            .grid_handler
            .url_at_point(Point { row: 0, col: 10 }),
