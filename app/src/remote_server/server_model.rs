...
use super::proto::{
    ...
    UploadHandoffSnapshotResponse, WriteFile, WriteFileResponse, WriteFileSuccess, client_message,
            ...
            Some(client_message::Message::HostScoped(wrapper)) => {
                let outcome = match wrapper.message {
                    ...
                    Some(host_scoped_request::Message::UploadHandoffSnapshot(_)) => {
                        // Heddle (FOSS): the local-to-cloud handoff snapshot upload is
                        // removed with the ambient runtime; there is no cloud target to
                        // upload to. Answer the request rather than leaving it hanging.
                        log::warn!(
                            "UploadHandoffSnapshot is not supported in this build (request_id={request_id})"
                        );
                        HandlerOutcome::Sync(
                            server_message::Message::UploadHandoffSnapshotResponse(
                                UploadHandoffSnapshotResponse {
                                    initial_snapshot_token: None,
                                    success: false,
                                    error: Some(
                                        "handoff snapshot upload is not supported in this build"
                                            .to_owned(),
                                    ),
                                },
                            ),
