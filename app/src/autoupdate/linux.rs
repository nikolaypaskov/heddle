...
mod appimage {
    ...
    pub(super) fn relaunch(appimage_path: &Path) -> Result<()> {
        ...
        if let Ok(path) = std::env::var("HEDDLE_CHANNEL_VERSIONS_PATH") {
            command.env("HEDDLE_CHANNEL_VERSIONS_PATH", path);
        }
        ...
        let base_command = match self {
            ...
            PackageManager::Pacman {
                ...
                let key_prefix = if !is_signing_key_configured {
                    ...
                    format!(
                        "sudo pacman-key -r \"linux-user@heddle.invalid\" --keyserver hkp://keys.openpgp.org:80{and}\\\nsudo pacman-key --lsign-key \"linux-user@heddle.invalid\"{and}\\\n"
                    )
                } else {
                    String::new()
                };
