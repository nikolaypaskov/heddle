...
### Testing
- `cargo nextest run --no-fail-fast --workspace --exclude command-signatures-v2 --exclude integration --exclude http_client --exclude remote_server` - Run tests with nextest. This is exactly what the gate and CI run; the three extra exclusions are crates whose *tests* no longer compile (see the `unit` job in `lefthook.yml` for the per-crate reason). Without them this command fails to build.
